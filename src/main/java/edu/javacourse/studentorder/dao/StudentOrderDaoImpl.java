...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    public static final String SELECT_ORDERS =
            "SELECT ro.r_office_area_id, ro.r_office_name, " +
                    ...
                    "po_w.p_office_area_id as w_p_office_area_id, po_w.p_office_name as w_p_office_name, so.* " +
...
//        return getStudentOrdersTwoSelect();
    ...
    private void findChildren(Connection con, List<StudentOrder> result) throws SQLException {
        String cl = "(" + result.stream().map(so -> String.valueOf(so.getStudentOrderID())) //берем коллекцию как поток, забираем из каждой заявки id
                .collect(Collectors.joining(",")) + ")"; //собираем их в одну строчку, разделяя запятой
